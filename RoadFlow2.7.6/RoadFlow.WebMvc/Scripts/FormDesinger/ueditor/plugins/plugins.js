...
//签章
UE.plugins['formsignature'] = function ()
{
    var me = this, thePlugins = 'formsignature';
    me.commands[thePlugins] = {
        execCommand: function ()
        {
            var dialog = new UE.ui.Dialog({
                iframeUrl: this.options.UEDITOR_HOME_URL + 'plugins/dialogs/signature.aspx',
                name: thePlugins + '_' + (new Date().valueOf()),
                editor: this,
                title: '签章',
    ...
    me.addListener('mouseover', function (t, evt)
    {
        ...
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', ''))
        {
            var html = popup.formatHtml('<nobr>签章: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html)
            {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
...
//弹出层选择
UE.plugins['formselectdiv'] = function ()
{
    var me = this, thePlugins = 'formselectdiv';
    me.commands[thePlugins] = {
        execCommand: function ()
        {
            var dialog = new UE.ui.Dialog({
                iframeUrl: this.options.UEDITOR_HOME_URL + 'plugins/dialogs/selectdiv.aspx',
                name: thePlugins + '_' + (new Date().valueOf()),
                editor: this,
                title: '弹出层选择',
                cssRules: "width:600px;height:400px;",
    ...
    me.addListener('mouseover', function (t, evt)
    {
        ...
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', ''))
        {
            var html = popup.formatHtml('<nobr>弹出层选择: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html)
            {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
...
//流水号
UE.plugins['formserialnumber'] = function ()
{
    var me = this, thePlugins = 'formserialnumber';
    me.commands[thePlugins] = {
        execCommand: function ()
        {
            var dialog = new UE.ui.Dialog({
                iframeUrl: this.options.UEDITOR_HOME_URL + 'plugins/dialogs/formserialnumber.aspx',
                name: thePlugins + '_' + (new Date().valueOf()),
                editor: this,
                title: '流水号',
                cssRules: "width:600px;height:300px;",
    ...
    me.addListener('mouseover', function (t, evt)
    {
        ...
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', ''))
        {
            var html = popup.formatHtml('<nobr>流水号: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html)
            {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
