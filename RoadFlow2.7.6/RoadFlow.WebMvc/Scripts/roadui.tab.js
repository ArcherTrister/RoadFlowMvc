...
;RoadUI.Tab = function (options)
{
    ...
    this.init = function ()
    {
        ...
        for (var i = 0; i < tabSize; i++)
        {
            ...
            var $li = $('<li class="tab_title_ul_li" ' + (clickevent ? 'clickevent="' + clickevent + '"' : '') + ' itemid="' + liID + '" close="' + (isClose ? "1" : "0") + '"></li>');
        ...
        if (instance.opts.contextmenu)
        {
            try
            {
                var baseurl = RoadUI.Core.rooturl();
    ...
    this.addTab = function (options)
    {
        ...
        else if (opts.src)
        {
            ...
            var iframeSrc = opts.src.toLowerCase().indexOf('tabid') >= 0 ? opts.src : opts.src.indexOf('?') > 0 ? opts.src + "&tabid=" + tabID : opts.src + "?tabid=" + tabID;
            iframeSrc = $.trim(iframeSrc).substr(0, 1) == "/" ? RoadUI.Core.rooturl() + iframeSrc : iframeSrc;
            var $iframe = $('<iframe id="' + tabID + '_iframe" name="' + tabID + '_iframe" frameborder="0" scrolling="auto" style="width:100%;' + (iframdHeight ? "height:" + (iframdHeight - 30).toString() + "px" : "") + '" src="' + iframeSrc + '"></iframe>');
