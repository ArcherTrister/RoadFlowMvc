	...
	public class MyController : Controller
	{
		...
		protected override void OnActionExecuting(ActionExecutingContext filterContext)
		{
			...
			else if (flag && !CheckLogin(out msg))
			{
				...
				{
					string str = System.Web.HttpContext.Current.Request.Url.PathAndQuery.UrlEncode();
					filterContext.Result = Content("<script>" + (msg.IsNullOrEmpty() ? "" : string.Format("alert('{0}');", msg)) + ((string.Compare(filterContext.Controller.ToString(), "WebMvc.Controllers.HomeController", true) == 0) ? ("top.location='" + base.Url.Content("~/Login") + "'") : ("top.lastURL='" + str + "';top.currentWindow=window;top.login();")) + "</script>", "text/html");
				}
			}
