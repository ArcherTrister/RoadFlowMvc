...
namespace RoadFlow.Data.ORACLE
{
	public class DBHelper : IDBHelper
	{
		...
		public DBHelper()
		{
			connectionString = Config.PlatformConnectionStringORACLE;
		}
		...
		public OracleDataReader GetDataReader(string sql)
		{
			...
			OracleConnection val = new OracleConnection(ConnectionString);
			((DbConnection)val).Open();
			OracleCommand val2 = new OracleCommand(sql, val);
			try
			{
		...
		public OracleDataReader GetDataReader(string sql, OracleParameter[] parameter)
		{
			...
			try
			{
				...
				OracleDataReader result = val2.ExecuteReader(CommandBehavior.CloseConnection);
		...
		public DataTable GetDataTable(string sql)
		{
			...
			{
				((DbConnection)val).Open();
				OracleDataAdapter val2 = new OracleDataAdapter(sql, val);
				try
				{
		...
		public DataTable GetDataTable(string sql, OracleParameter[] parameter)
		{
			...
			try
			{
				...
				try
				{
					...
					OracleDataAdapter val3 = new OracleDataAdapter(val2);
					try
					{
						DataTable dataTable = new DataTable();
		...
		public int Execute(List<string> sqlList)
		{
			...
			{
				((DbConnection)val).Open();
				OracleCommand val2 = new OracleCommand();
				try
				{
					int num = 0;
		...
		public int Execute(string sql, OracleParameter[] parameter)
		{
			...
			try
			{
				...
				try
				{
					...
					int result = ((DbCommand)val2).ExecuteNonQuery();
					((DbParameterCollection)val2.get_Parameters()).Clear();
					((DbCommand)val2).Prepare();
					return result;
				}
		...
		public string GetFields(string sql, OracleParameter[] param)
		{
			...
			try
			{
				...
				try
				{
					...
					OracleDataReader val3 = val2.ExecuteReader(CommandBehavior.SchemaOnly);
		...
		public string GetPaerSql(string table, string fileds, string where, string order, int size, int number, out long count, OracleParameter[] param = null)
		{
			...
			string value = string.Format("SELECT PagerTempTable.*,ROWNUM AS PagerAutoRowNumber FROM (SELECT {0} FROM {1} {2} ORDER BY {3}) PagerTempTable", fileds, table, text, order);
		...
		public int DataTableToDB(DataTable dt)
		{
			...
			{
				((DbConnection)val).Open();
				OracleDataAdapter val2 = new OracleDataAdapter("select * from " + dt.TableName + " where 1=0", val);
				try
				{
					DataTable dataTable = new DataTable();
					((DbDataAdapter)val2).Fill(dataTable);
